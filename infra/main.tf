...
resource "aws_vpc" "main" {
  ...
  tags = { Name = "paper-to-notebook-vpc" }
}
...
resource "aws_internet_gateway" "igw" {
  vpc_id = aws_vpc.main.id
  tags   = { Name = "paper-to-notebook-igw" }
}
...
resource "aws_lb" "main" {
  ...
  tags = { Name = "paper-to-notebook-alb" }
}
...
resource "aws_ecs_task_definition" "backend" {
  ...
  container_definitions = jsonencode([{
    ...
    logConfiguration = {
      logDriver = "awslogs"
      options = {
        "awslogs-group"         = "/ecs/paper-to-notebook/backend"
...
resource "aws_ecs_task_definition" "frontend" {
  ...
  container_definitions = jsonencode([{
    ...
    logConfiguration = {
      logDriver = "awslogs"
      options = {
        "awslogs-group"         = "/ecs/paper-to-notebook/frontend"
        "awslogs-region"        = var.region
...
resource "aws_cloudwatch_log_group" "backend" {
  name              = "/ecs/paper-to-notebook/backend"
  retention_in_days = 7
}

resource "aws_cloudwatch_log_group" "frontend" {
  name              = "/ecs/paper-to-notebook/frontend"
  retention_in_days = 7
}
